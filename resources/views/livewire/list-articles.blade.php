<div>
            ...
            @if ($articles->count())
                <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
                    <thead class="bg-gray-50">
                        <tr>
                            <th wire:click="order('id')" scope="col" class="w-24 px-6 py-4 font-medium text-gray-900 cursor-pointer">ID
                                @if ($sort == 'id')
                                    @if ($direction == 'asc')
                            ...
                            <th wire:click="order('name')" scope="col" class="px-6 py-4 font-medium text-gray-900 cursor-pointer"><b>Nombre</b>
                                @if ($sort == 'name')
                                    @if ($direction == 'asc')
                                        <i class="fas fa-sort-alpha-up-alt float-right"></i>        
                                    @else
                            ...
                            <th wire:click="order('description')" scope="col" class="px-6 py-4 font-medium text-gray-900 cursor-pointer"><b>Descripción</b>
                                @if ($sort == 'description')
                                    @if ($direction == 'asc')
                                        <i class="fas fa-sort-alpha-up-alt float-right"></i>        
                                    @else
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <th class="flex gap-3 px-6 py-4 font-normal text-gray-900">
                                    <div class="relative h-10 w-10">
                                        <img wire:click="show({{ $item }})" class="h-full w-full rounded-full object-cover object-center cursor-pointer"
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex justify-end gap-4">
                                        ...
                                        <a wire:click="edit({{ $item }})" x-data="{ tooltip: 'Edite' }" href="#">
    ...
    <x-jet-dialog-modal wire:model="open_edit">
        ...
        <x-slot name="content">
            ...
            <div class="mb-4">
                ...
                <textarea wire:model="article.description" rows="6" class="form-control w-full" placeholder="Escriba una descripción"></textarea>
            </div>
            <div class="mb-4">
                ...
                <select wire:model="article.laboratory_id" class="form-control w-full">
                    ...
                    @foreach ($laboratories as $laboratory)
                        <option value="{{ $laboratory->id }}" {{ old('laboratory->id') == 'laboratory->id' ? 'selected' : '' }} >{{ $laboratory->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
    ...
    <x-jet-modal wire:model="open_image">
        <div class="img-container">
              <div class="m-2">
                <img src="{{ Storage::url($article->image) }}" class="max-w-full h-auto rounded-full" alt="">
              </div>
        </div>
    </x-jet-modal>
    {{-- Modal ver imagen /--}}
